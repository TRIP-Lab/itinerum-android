...
public class PausePlayDrawable extends Drawable {

	private static final Property<PausePlayDrawable, Float> PROGRESS =
			new Property<PausePlayDrawable, Float>(Float.class, "progress") {
				@Override
				public Float get(PausePlayDrawable d) {
					return d.getProgress();
				}

				@Override
				public void set(PausePlayDrawable d, Float value) {
					d.setProgress(value);
				}
			};
	...
	public PausePlayDrawable(Context context, int size) {
		final Resources res = context.getResources();

		boolean isSmall = size == FloatingActionButton.SIZE_MINI;

		mPaint.setAntiAlias(true);
		mPaint.setStyle(Paint.Style.FILL);
		mPaint.setColor(Color.WHITE);
		mPauseBarWidth = isSmall ? res.getDimensionPixelSize(R.dimen.pause_bar_width)/2 : res.getDimensionPixelSize(R.dimen.pause_bar_width);
		mPauseBarHeight = isSmall ? res.getDimensionPixelSize(R.dimen.pause_bar_height)/2 : res.getDimensionPixelSize(R.dimen.pause_bar_height);
		mPauseBarDistance = isSmall ? res.getDimensionPixelSize(R.dimen.pause_bar_distance)/2 : res.getDimensionPixelSize(R.dimen.pause_bar_distance);
	}
